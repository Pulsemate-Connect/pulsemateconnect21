...
function DoctorAvatar({ photoUrl, name, size = 72 }) {
  ...
  if (photoUrl && !broken) {
    return (
      <Image
        ...
        onError={(error) => {
          console.log(`[DoctorAvatar] Image load failed for ${name}:`, {
            url: photoUrl,
        ...
        onLoad={() => {
          console.log(`[DoctorAvatar] Image loaded successfully for ${name}: ${photoUrl}`);
        }}
        resizeMode="cover"
      />
    );
  ...
  if (broken && photoUrl) {
    console.log(`[DoctorAvatar] Showing fallback for ${name}, photo URL was: ${photoUrl}`);
  }
    ...
    <View style={[s.card, cardWidth ? { width: cardWidth } : null]}>
      ...
      {rating && (
        <View style={s.ratingRow}>
          ...
          {ratingCount && <Text style={s.ratingCount}>({ratingCount})</Text>}
        </View>
      )}
