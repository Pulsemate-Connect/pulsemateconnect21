...
export default function HomeHeader({ onLocationFetched }) {
  ...
  const handleModalSelect = useCallback((item) => {
    ...
    // If item has coords (GPS pick or geocoded), pass them to parent
    if (item.latitude && item.longitude) {
      console.log('[HomeHeader] Location selected with coords:', item.name, item.latitude, item.longitude);
      onLocationFetched?.({ latitude: item.latitude, longitude: item.longitude });
    } else {
      console.log('[HomeHeader] Location selected without coords:', item.name);
    }
  }, [onLocationFetched]);
