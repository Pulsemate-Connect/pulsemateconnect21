...
export default function BookingScreen({ route, navigation }) {
  ...
  const handleBook = async () => {
    // ✅ CHECK 1: Authentication - Must be logged in to book
    if (!user || !token) {
      Alert.alert(
        'Login Required',
        'Please login to book an appointment. Your booking details will be saved.',
        [
          ...
          { 
            ...
            onPress: () => {
              ...
              // The user will need to complete login and then navigate back to booking
              navigation.navigate('Auth', { 
                screen: 'Login',
                params: {
                  // Pass booking details to resume after login
                  returnTo: 'Booking',
                  bookingParams: { doctorId, clinicId, doctorName, clinicName, fee, specialization, doctorPhoto }
                }
              });
            }
          },
  ...
  function BookingDoctorAvatar() {
    ...
    if (__DEV__) {
      console.log('[BookingScreen] Avatar data:', {
        doctorPhoto,
        doctorName,
        broken
      });
    ...
    if (doctorPhoto && !broken) {
      return (
        <Image
          ...
          onError={(e) => {
            if (__DEV__) {
              console.error('[BookingScreen] Avatar image failed to load:', {
                uri: doctorPhoto,
                error: e.nativeEvent
              });
            }
            ...
            <View style={s.doctorInfo}>
              <View style={s.doctorNameRow}>
                <Text style={s.doctorName}>
                  {doctorName?.startsWith('Dr.') ? doctorName : `Dr. ${doctorName || 'Doctor'}`}
                </Text>
