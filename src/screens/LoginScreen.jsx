...
`);
    ...
    if (!trimmed || trimmed.length === 0) {
      console.warn('[LoginScreen] ⚠️  Phone number is empty');
    ...
    if (trimmed.length > 10) {
      console.warn('[LoginScreen] ⚠️  Mobile number too long:', trimmed.length);
      Alert.alert('Invalid Number', 'Mobile number should be exactly 10 digits');
      return;
    }
    
    // Validate that it's all digits
    if (!/^\d{10}$/.test(trimmed)) {
      console.warn('[LoginScreen] ⚠️  Mobile number contains non-digits');
      Alert.alert('Invalid Number', 'Please enter only digits (0-9)');
      return;
    }
        ...
        <View style={s.formCard}>
          ...
          <View style={[s.inputRow, focused && s.inputRowFocused]}>
            ...
            <TextInput
              ...
              value={mobile}
              onChangeText={(text) => {
                console.log('[LoginScreen] Input changed:', text);
                
                // Remove all non-digits
                let digits = text.replace(/\D/g, '');
                console.log('[LoginScreen] After removing non-digits:', digits);
                
                // If user typed 91 at the beginning, remove it (they think it's country code)
                if (digits.startsWith('91') && digits.length > 10) {
                  console.log('[LoginScreen] Detected 91 prefix with >10 digits, removing prefix');
                  digits = digits.slice(2);
                }
                
                // Also handle if they type just 91 followed by actual number
                // E.g., if they type "919876543210" (12 digits), extract the last 10
                if (digits.length === 12 && digits.startsWith('91')) {
                  console.log('[LoginScreen] Detected 12 digits starting with 91, removing prefix');
                  digits = digits.slice(2);
                }
                ...
                console.log('[LoginScreen] Final digits:', digits, '(length:', digits.length + ')');
                setMobile(digits);
              }}
...
const s = StyleSheet.create({
  ...
  helperText:      { fontSize: 11, color: BLUE, marginTop: -6, marginBottom: 10, paddingHorizontal: 4, fontWeight: '600' },
