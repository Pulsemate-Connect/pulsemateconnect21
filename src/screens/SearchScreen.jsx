...
// Note: 'key' must match the exact specialization value stored in the database
const SPECS = [
  { key: 'All',              label: 'All',              icon: 'apps',          color: SKY5,     bg: '#E0F2FE' },
  { key: 'General Physician',label: 'General Physician',icon: 'medkit',        color: '#6366F1', bg: '#EEF2FF' },
  { key: 'Cardiology',       label: 'Cardiology',       icon: 'heart',         color: '#EF4444', bg: '#FEE2E2' },
  { key: 'Dermatology',      label: 'Dermatology',      icon: 'color-palette',color: '#F59E0B', bg: '#FEF3C7' },
  { key: 'Orthopedics',      label: 'Orthopedics',      icon: 'body',          color: '#10B981', bg: '#D1FAE5' },
  { key: 'Pediatrics',       label: 'Pediatrics',       icon: 'happy',         color: '#EC4899', bg: '#FCE7F3' },
  { key: 'Neurology',        label: 'Neurology',        icon: 'pulse',         color: '#8B5CF6', bg: '#EDE9FE' },
  { key: 'Physiotherapy',    label: 'Physiotherapy',    icon: 'fitness',       color: '#0EA5E9', bg: '#E0F2FE' },
  { key: 'Dentistry',        label: 'Dentistry',        icon: 'happy-outline', color: '#14B8A6', bg: '#CCFBF1' },
        ...
        <View style={dc.infoCol}>
          ...
          <View style={dc.nameRow}>
            <Text style={dc.name} numberOfLines={1}>
              {doc.user?.name?.startsWith('Dr.') || doc.user?.name?.startsWith('Dr ') 
                ? doc.user.name 
                : `Dr. ${doc.user?.name}`}
            </Text>
...
export default function SearchScreen({ navigation }) {
  ...
  // ✅ BUG FIX #003: Run initial search only once on mount
  // Use useRef to prevent running on every doSearch change
  const hasInitialized = useRef(false);
  useEffect(() => {
    if (!hasInitialized.current) {
      hasInitialized.current = true;
      doSearch({ spec: 'All', query: '' });
    }
  }, [doSearch]);
