...
const portalCards = [
  {
    title: 'Doctor Login',
    description: 'Access your practice dashboard, manage appointments, and view patient queue.',
    href: '/doctor/login',
    icon: '🩺',
    gradient: 'from-blue-500 to-blue-600',
    features: ['View Appointments', 'Manage Queue', 'Digital Prescriptions'],
  },
  {
    title: 'Staff Login',
    description: 'Manage clinic operations, book appointments, and handle patient registration.',
    href: '/staff/login',
    icon: '👥',
    gradient: 'from-emerald-500 to-teal-600',
    features: ['Book Appointments', 'Queue Management', 'Patient Records'],
  },
];
            ...
            <div className="grid gap-10 xl:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)] xl:items-start 2xl:gap-14 2xl:grid-cols-[minmax(0,0.86fr)_minmax(0,1.14fr)]">
              <div className="min-w-0 animate-fade-up">
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  <Link
                    to="/doctor/login"
                    className="inline-flex items-center justify-center rounded-2xl bg-[linear-gradient(135deg,#8b5cf6_0%,#a855f7_100%)] px-5 py-3 text-sm font-semibold text-white shadow-[0_16px_35px_rgba(139,92,246,0.2)] transition hover:brightness-105"
                  >
                    Doctor Login
                  </Link>
                  <Link
                ...
                <div className="mt-5 rounded-2xl border border-blue-100 bg-[linear-gradient(180deg,#f5f8ff_0%,#eef4ff_100%)] px-4 py-4 text-blue-700 shadow-sm">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-sm font-medium sm:text-base">
                      Doctor and receptionist accounts are created by clinic administration.
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="grid gap-5 md:grid-cols-2">
                    {portalCards.map((card, index) => (
                      <Link
                        key={card.title}
                        to={card.href}
                        className={`group relative overflow-hidden rounded-[1.8rem] border border-white/20 bg-gradient-to-br ${card.gradient} p-8 text-white shadow-[0_20px_50px_rgba(37,99,235,0.15)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_25px_60px_rgba(37,99,235,0.25)]`}
                      >
                        {/* Icon */}
                        ...
                        {/* Description */}
                        <p className="text-white/90 text-base leading-relaxed mb-5">{card.description}</p>
                        
                        {/* Features List */}
                        <div className="space-y-2 mb-6">
                          {card.features.map((feature, idx) => (
                            <div key={idx} className="flex items-center gap-2 text-sm text-white/95">
                        ...
                        {/* Button */}
                        <div className="inline-flex items-center gap-2 rounded-xl bg-white/20 backdrop-blur-sm px-4 py-2.5 text-sm font-semibold text-white transition group-hover:bg-white/30">
