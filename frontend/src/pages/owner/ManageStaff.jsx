...
const ManageStaff = ({ staffRole = 'DOCTOR' }) => {
  ...
  const [addForm, setAddForm] = useState({ mobile: '', name: '', email: '', specialization: '', password: '' });
  ...
  }, [selectedClinic?.id, staffRole]);

  const fetchDoctorInvites = useCallback(async () => {
    if (!selectedClinic?.id || staffRole !== 'DOCTOR') return;
    try {
  ...
  }, [selectedClinic]); // Only depend on selectedClinic to prevent infinite loop
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedClinic, staffRole]); // Remove function dependencies to prevent infinite loop
  ...
  const handleAddStaff = async (e) => {
    ...
    try {
      if (staffRole === 'DOCTOR') {
        ...
        setAddForm({ mobile: '', name: '', email: '', specialization: '', password: '' });
      ...
      const errorMsg = err.response?.data?.errors?.[0]?.message || err.response?.data?.message || 'Failed to add staff';
