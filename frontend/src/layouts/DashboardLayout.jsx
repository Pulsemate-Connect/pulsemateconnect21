...
const NAV_ITEMS = {
  SUPER_ADMIN: [
    ...
    { path: '/admin/doctors',        label: 'Doctors',       icon: Icon.User     },
...
const DashboardLayout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Only poll if page is visible (stop polling when user switches tabs)
    const startPolling = () => {
      interval = setInterval(fetchUnread, 120_000); // Poll every 2 minutes (less aggressive)
    };
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        fetchUnread(); // Fetch immediately when tab becomes visible
        startPolling();
      }
    };
  ...
  // Only refresh when navigating TO notifications page (not every navigation)
  useEffect(() => {
    if (!user) return;
