...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow p-2 text-sm">
      ...
      <p style={{ color: '#2563EB' }}>{payload[0].value} patients</p>
    </div>
  );
};

export function PatientDemographicsBar({ data = [], loading, empty, isMobile }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={260}>
      <BarChart data={data} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
        ...
        {!isMobile && <Legend />}
        <Bar dataKey="count" fill="#2563EB" name="Patients" radius={[3, 3, 0, 0]} />
      </BarChart>
    </ResponsiveContainer>
  );
}

export default PatientDemographicsBar;
