...
const globalLimiter = rateLimit({
  ...
  max: 100,                 // 100 requests per minute PER USER (not per IP)
  standardHeaders: true,
  legacyHeaders: false,
  message: { success: false, message: 'Too many requests, please try again later.' },
  
  // ✅ CAPACITY FIX: Rate limit per authenticated user, not per IP
  // This allows multiple users from the same IP (load tests, clinic networks)
  keyGenerator: (req) => {
    ...
    // If user is already authenticated (e.g., by earlier middleware)
...
// Serve uploaded files with explicit CORS headers so browsers on pulsemateconnect.in
// can load images from api.pulsemateconnect.in/uploads without NotSameOrigin errors
...
if (process.env.NODE_ENV !== 'test') {
  server.listen(PORT, '0.0.0.0', async () => {
    ...
    startReminderJob();
    startNoShowJob(); // Auto-cancel no-show appointments after grace period
