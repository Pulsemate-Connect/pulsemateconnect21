 ...
 * In development, uses the local network IP for mobile access.
 */
const getFileUrl = (filename, req) => {
  ...
  // In development, use the IP address instead of localhost for mobile access
  if (process.env.NODE_ENV === 'development') {
    // Use the local network IP (same as backend API)
    return `http://192.168.31.240:5000/uploads/${filename}`;
  }
