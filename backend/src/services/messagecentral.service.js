...
async function sendOTP(mobileNumber, otpLength = 6) {
  try {
    // Clean mobile number (remove any non-digits first)
    let cleanNumber = mobileNumber.replace(/\D/g, '');
    
    // Only remove leading 91 if the total length is 12 (country code + 10 digits)
    // This handles both +919876543210 and 919876543210
    // But preserves numbers like 9110650296 (which start with 91 but are only 10 digits)
