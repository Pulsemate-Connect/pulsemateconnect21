...
const addStaff = async (req, res, next) => {
  try {
    const { id: clinicId } = req.params;
    let { mobile, role, name, email, password } = req.body;
    ...
    // Clean mobile number (remove spaces, dashes) - this happens AFTER validation
    if (mobile) {
      const originalMobile = mobile;
      mobile = mobile.replace(/[\s-]/g, '');
      console.log('[AddStaff] Mobile cleaned:', { original: originalMobile, cleaned: mobile });
    }
    ...
    console.log('[AddStaff] Success:', { userId: user.id, role, mobile });
