...
/**
 * Helper: Recalculate positions AND estimated appointment times for all waiting items.
 * Accounts for doctor delay: if the current consultation started late,
 * all remaining appointments shift forward by the accumulated delay.
 */
const recalculatePositions = async (queueId, doctorAvgMins = 10) => {
  ...
  // Calculate accumulated delay from current consultation
  let delayMins = 0;
  ...
  for (let i = 0; i < waitingItems.length; i++) {
    ...
    const estimatedWaitMinutes = newPosition * doctorAvgMins;

    // Estimated appointment time: use slotTime + delay if available
    let newEstimatedTime = null;
    if (item.appointment?.slotTime && delayMins > 0) {
      ...
      updates.push(
        prisma.appointment.update({
          ...
          data: {
            ...
            // Only update slotTime-derived estimates if there's a delay
            ...(newEstimatedTime ? {} : {}),
          },
        })
      );
...
const getQueue = async (req, res, next) => {
  try {
    ...
    // ✅ BUG #007 FIX: Validate clinic access before returning queue data
    // Prevent receptionists from accessing other clinics' queues
...
/**
 * POST /api/reception/walk-in-batch - Bulk add walk-in patients
 * Optimized for high-traffic scenarios (up to 50 patients at once)
 */
const addWalkInBatch = async (req, res, next) => {
  try {
    const { patients, doctorId, clinicId, sessionId } = req.body;
    
    // Validation
    if (!Array.isArray(patients)) {
      return sendError(res, 'patients must be an array', 400);
    }
    
    if (patients.length === 0) {
      return sendError(res, 'At least one patient is required', 400);
    }
    
    if (patients.length > 50) {
      return sendError(res, 'Maximum 50 patients can be added at once', 400);
    }
    
    if (!doctorId || !clinicId) {
      return sendError(res, 'doctorId and clinicId are required', 400);
    }
    ...
    for (let i = 0; i < patients.length; i += 10) {
      ...
      const batchPromises = batch.map(async (patientData) => {
        try {
          const { mobile, name, symptoms } = patientData;
          ...
          // ✅ BUG FIX #001: Atomic queue number assignment with advisory lock
          // This prevents race conditions when multiple receptionists add walk-ins simultaneously
          const result = await prisma.$transaction(async (tx) => {
            // Use PostgreSQL advisory lock
            const crypto = require('crypto');
            const hash = crypto.createHash('sha256').update(queue.id).digest();
    ...
    }, `Batch processing complete: ${results.length} successful, ${errors.length} failed`, 201);
...
const addWalkIn = async (req, res, next) => {
  try {
    ...
    // ✅ BUG #010 FIX: Use upsert to prevent duplicate patient creation race condition
    // When 2 receptionists add same mobile simultaneously, only 1 user is created
    let patient;
    try {
      patient = await prisma.user.upsert({
      ...
      // Handle duplicate mobile number (race condition with unique constraint)
      if (createError.code === 'P2002' && createError.meta?.target?.includes('mobile')) {
        // Another receptionist created this patient simultaneously - fetch it
        ...
        if (!patient) {
          return sendError(res, 'Unable to create patient. Please try again.', 500);
        }
        
        console.log(`[addWalkIn] Patient with mobile ${patientMobile} already exists, using existing record`);
      } else {
        throw createError;
      }
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      try {
        if (sessionId) {
          const sess = await tx.clinicSession.findUnique({ where: { id: sessionId } });
          if (sess) {
      ...
      return { appointment, queueItem, queueNumber, estimatedAppointmentTimeWalkIn };
    }, { isolationLevel: 'Serializable', timeout: 10000 });
    
    const { appointment, queueItem, queueNumber, estimatedAppointmentTimeWalkIn } = result;
...
const addFollowUp = async (req, res, next) => {
  try {
    ...
    // ✅ BUG FIX #001: Atomic queue number + follow-up insertion with advisory lock
    const result = await prisma.$transaction(async (tx) => {
      // Use PostgreSQL advisory lock
      const crypto = require('crypto');
      const hash = crypto.createHash('sha256').update(queue.id).digest();
      ...
      const followUpWaitingCount = await tx.queueItem.count({
        where: { queueId: queue.id, status: 'WAITING', isFollowUp: true },
      });

      const position = followUpWaitingCount + 1;
      ...
      return { followUpAppointment, queueItem, queueNumber };
    }, { isolationLevel: 'Serializable', timeout: 10000 });
...
const callNext = async (req, res, next) => {
  try {
    ...
    // Send FCM push notification to the called patient - persist in database
    createNotification({
      userId: nextItem.patientId,
      type: 'QUEUE_CALLED',
      title: '🔔 Your Turn!',
      message: `Queue #${nextItem.queueNumber} — Please proceed to the doctor's room.`,
      metadata: {
        queueId,
...
const pauseQueue = async (req, res, next) => {
  try {
    ...
    // Notify all waiting patients - persist in database
    const waitingItems = await prisma.queueItem.findMany({
      where: { queueId, status: 'WAITING' },
    ...
    // Create notification for each waiting patient
    for (const patientId of patientIds) {
      createNotification({
        userId: patientId,
        type: 'QUEUE_PAUSED',
        title: '⏸️ Queue Paused',
        message: `Dr. ${doctorName}'s queue has been temporarily paused. Please wait.`,
        metadata: {
          queueId,
          clinicId: queue.clinicId,
          doctorId: queue.doctorId,
...
const resumeQueue = async (req, res, next) => {
  try {
    ...
    // Notify all waiting patients that queue has resumed - persist in database
    try {
      ...
      for (const patientId of uniquePatientIds) {
        createNotification({
          ...
          message: `Dr. ${doctorName}'s queue has resumed. Please come back to the clinic.`,
          metadata: {
            queueId,
            clinicId: resumedQueue.clinicId,
            doctorId: resumedQueue.doctorId,
          },
          priority: 'HIGH',
        }).catch(() => { });
      }
